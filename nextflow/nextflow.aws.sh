...
DEFAULT_AWS_CLI_PATH=/usr/local/aws-cli/v2/current/bin/aws
...
if [[ "$NEXTFLOW_PROJECT" =~ ^s3://.* ]]; then
    ...
    aws s3 sync --no-progress $NEXTFLOW_PROJECT ./project
    NEXTFLOW_PROJECT=./project
fi

# Create the default config using environment variables
# passed into the container
NF_CONFIG=project/nextflow.config
echo "Created/Adding to config: $NF_CONFIG"
